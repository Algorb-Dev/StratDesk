...
export const FaqAccordion: React.FC<{ defaultCategory?: string }> = ({
  ...
  return (
    <section id="faq" className="relative py-28 border-b border-white/10 bg-background">
            ...
            <button
              ...
              className={cn(
                ...
                selectedCategory === cat
                  ? "bg-accent/15 border-accent/60 text-accent font-bold shadow-glow-cyan"
          ...
          {filteredFaqs.map((faq) => {
            ...
            return (
              <div
                key={faq.id}
                className={cn(
                  "rounded-xl border transition-all duration-200 overflow-hidden bg-surface/70",
                  isOpen ? "border-white/25 bg-surface-elevated/80 shadow-lg" : "border-white/10 hover:border-white/20"
                )}
              >
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] text-accent uppercase tracking-wider px-1.5 py-0.5 rounded bg-accent/10 border border-accent/20">
                      {faq.category}
                    </span>
                ...
                {isOpen && (
                  <div className="px-4 pb-5 sm:px-5 border-t border-white/5 pt-3">
                    <p className="text-xs sm:text-sm text-text-secondary font-sans leading-relaxed">
                      {faq.answer}
                    </p>
                    {faq.codeSnippet && (
                      <div className="mt-3 p-3 rounded bg-black/60 border border-white/10 text-[11px] text-accent font-mono overflow-x-auto">
                        <pre>
