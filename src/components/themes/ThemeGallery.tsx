...
export const ThemeGallery: React.FC = () => {
  ...
  return (
    <section id="themes" className="relative py-28 border-b border-white/10 bg-background-secondary overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="mt-4 text-base sm:text-lg text-text-secondary leading-relaxed font-sans">
            Personalize your command center to match your workstation aesthetic. Every Algorb interface includes 6 precision-crafted color and typography themes.
          </p>
        </div>
          ...
          {THEMES.map((theme) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "p-3.5 rounded-xl text-left border transition-all duration-200 flex flex-col justify-between group",
                  isSelected
                    ? "bg-surface-elevated border-accent shadow-glow-cyan"
                    : "bg-surface/50 border-white/10 hover:border-white/25"
                )}
              >
        ...
        <div className="rounded-2xl border border-white/15 bg-background p-4 sm:p-8 shadow-2xl flex flex-col gap-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-white/10 font-mono">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xl font-bold text-white font-sans">{activeTheme.name}</span>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {activeTheme.traits.map((trait) => (
                <span
                  key={trait}
                  className="px-2 py-1 rounded bg-white/5 border border-white/10 text-[10px] text-text-muted"
                >
                  {trait}
                </span>
              ))}
